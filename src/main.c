...
void clear_on_input_cb(GtkEditable *editable, const gchar *new_text,
                       gint new_text_length, gint *position,
                       gpointer user_data) {
  GtkBuilder *builder = GTK_BUILDER(user_data);
  GtkEntry *entry = GTK_ENTRY(gtk_builder_get_object(builder, "calc_display"));
  GtkTextView *error_box =
      GTK_TEXT_VIEW(gtk_builder_get_object(builder, "calc_error_box"));
...
void eval_cb(GSimpleAction *action, GVariant *parameter, gpointer user_data) {
  GtkBuilder *builder = GTK_BUILDER(user_data);
  GtkEntry *entry = GTK_ENTRY(gtk_builder_get_object(builder, "calc_display"));
  GtkTextView *error_box =
      GTK_TEXT_VIEW(gtk_builder_get_object(builder, "calc_error_box"));
  ...
  const gchar *og_input = gtk_entry_get_text(entry);
  char *input = g_strdup(og_input);
  remove_whitespace(input);

  token_t result;
  err_t err = eval_expression(input, &result);
  if (err != OK) {
    gtk_text_buffer_set_text(buffer, get_error_message(err), -1);
    error_shown = TRUE;
  } else {
    if (result.val.operand.type == DECIMAL) {
      gtk_entry_set_text(entry,
                         g_strdup_printf("%.2lf", result.val.operand.val.dec));
    } else {
      gtk_entry_set_text(
          entry, g_strdup_printf("0x%lx", result.val.operand.val.hex_bin));
    }
    result_shown = TRUE;
  }
}
...
void toggle_op_pad_cb(GSimpleAction *action, GVariant *state,
  ...
  gtk_stack_set_visible_child_name(op_stack, is_bitwise ? "bitwise_op_pad"
                                                        : "arithmetic_op_pad");
...
int main() {
  ...
  if (gtk_builder_add_from_resource(builder, "/asc91/dumb-calc/calc-gtk.ui",
                                    &error) == 0) {
  ...
  GSimpleAction *all_clear = g_simple_action_new("all_clear", NULL);
  ...
  GSimpleAction *del_buffer = g_simple_action_new("del_buffer", NULL);
  ...
  g_action_map_add_action(G_ACTION_MAP(GTK_APPLICATION_WINDOW(window)),
                          G_ACTION(eval));
  g_signal_connect(GTK_ENTRY(entry), "activate", G_CALLBACK(enter_key_cb),
                   builder);
  ...
  GSimpleAction *toggle_op_pad =
      g_simple_action_new_stateful("toggle_op_pad", NULL, toggle_state);
  g_signal_connect(toggle_op_pad, "change-state", G_CALLBACK(toggle_op_pad_cb),
                   op_stack);
  g_action_map_add_action(G_ACTION_MAP(GTK_APPLICATION_WINDOW(window)),
                          G_ACTION(toggle_op_pad));
  g_signal_connect(window, "destroy", G_CALLBACK(gtk_main_quit), NULL);

  g_signal_connect(GTK_ENTRY(entry), "insert-text",
                   G_CALLBACK(clear_on_input_cb), builder);
  gtk_main();
  return 0;
}
